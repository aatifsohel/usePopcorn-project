...
*/
  ...
  function handleSelectMovie(id) {
    // if selectedId is equal to id that is already in movieDetails then set selected id to null
    setSelectedId(currentSelectedID => (id === currentSelectedID ? null : id));
  }
    ...
    function () {
      ...
      async function fetchMovies() {
        try {
          ...
          // checking if the response is string 'False' not boolean - from API
      ...
      // if character < 3 - don't fetch data
      if (query.length < 3) {
        setMovies([]);
        setErrorText("");
        return;
...
function MovieDetails({
  ...
  // Derived state - creating an array based on imdbID & checking if includes the current selected ID
  const isWatched = watched.map(movie => movie.imdbID).includes(selectedId);

  // if userRating in watchedMovieslist then take userRating
  ...
  function handleAddToList() {
    ...
    // Adding to watched list & closing it after it has been added
        ...
        <>
          ...
          <section>
            <div className="rating">
              {/* If already watched/rated don't display stars */}
              {!isWatched ? (
                <>
                  {" "}
                  <StarRating
                  ...
                  />
                  {/* if user has rated the movie then show add to list */}
                  {userRating > 0 && (
